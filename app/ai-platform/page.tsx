...
import {
  Code, Globe, Brain, Cpu, Shield, BarChart3,
  MessageSquare, Zap, Database, Server, ArrowDown,
  Clock, TrendingDown, CheckCircle2, Sparkles, Activity, Layers
} from "lucide-react"
...
const scenarios = [
  {
    ...
    subtitle: "200+ engineers across Shenzhen · Singapore · Bangalore",
    model: "Claude Sonnet 4.6 (complex generation) + Haiku 4.5 (fast completion)",
    dailyTokens: "~580M tokens/day",
    techStack: "Cocos Creator (TypeScript), Unity (C#), Go, Python",
    capabilities: [
      { label: "Smart Code Review", detail: "PR review time: 4.2h → ≤1.5h (↓64%)" },
      { label: "Bug Diagnosis", detail: "Localization time: 3.5h → ≤1h (↓71%)" },
      { label: "AI Code Generation", detail: "IDE completion for VS Code / JetBrains" },
      { label: "Auto Test Generation", detail: "Unit & integration test scaffolding" },
    ],
  },
  {
    id: "customer",
  ...
  },
  {
    ...
    capabilities: [
      { label: "Card Balance Analysis", detail: "Adjustment cycle: 3 weeks → ≤3 days (↓86%)" },
      { label: "Anti-Cheat System", detail: "False positive: 5% → 0.5% (↓90%)" },
      { label: "Operations Copilot", detail: "Analysis: 3 days → 5 min (NL→SQL→Report)" },
      { label: "NPC Dynamic Dialogue", detail: "3M calls/day, <800ms, 10+ languages" },
...
const comparisonData = [
  { scenario: "Multilingual Intent Recognition (6 langs, 1200 samples)", claude: "92.6%", qwen: "73.7%", gap: "+18.9pp" },
  { scenario: "Code Generation Accuracy (450 tasks)", claude: "84.8%", qwen: "61.0%", gap: "+23.8pp" },
  { scenario: "Long-Context Bug Diagnosis (20 cases)", claude: "85.0%", qwen: "35.0%", gap: "+50.0pp" },
  { scenario: "Content Moderation Throughput (10K items)", claude: "85/s, 94.2%", qwen: "45/s, 78.5%", gap: "+89% throughput" },
  { scenario: "Card Balance Reasoning (5 cases)", claude: "4.2/5", qwen: "2.8/5", gap: "+1.4 pts" },
  { scenario: "Weighted Overall Score", claude: "92.3", qwen: "56.5", gap: "+35.8", highlight: true },
]

const architecture = [
  { layer: "Access Layer", detail: "CloudFront + ALB (Dual AZ)", icon: Globe },
  { layer: "Routing Layer", detail: "GameAI Hub — Smart routing (95% → Haiku, 5% → Sonnet)", icon: Zap },
  { layer: "AI Layer", detail: "7 Bedrock Agents + Knowledge Bases (RAG) + OpenSearch", icon: Brain },
  { layer: "Cache Layer", detail: "ElastiCache Redis (response cache hit rate 45%+)", icon: Database },
  { layer: "Data Layer", detail: "Aurora MySQL + S3", icon: Server },
]

const timeline = [
  { phase: "Phase 0", time: "2026/05", content: "AI Readiness Assessment", mrr: "—" },
  { phase: "Phase 1 MVP", time: "2026/06-07", content: "AI Coding + India Customer Service", mrr: "~$50K" },
  { phase: "Phase 2 Expansion", time: "2026/07-09", content: "Full-language CS + Moderation + Anti-Cheat", mrr: "~$150K" },
  { phase: "Phase 3 Full Scale", time: "2026/10-11", content: "All scenarios live + optimization", mrr: "~$200K" },
]
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-white">
        ...
        <div className="container mx-auto px-6 lg:px-12 relative z-10 pt-24">
          ...
          >
            ...
            >
              Processing{" "}
              <span className="font-semibold text-primary">14.9 billion tokens daily</span>{" "}
              across 3 core AI scenarios with{" "}
              <span className="font-semibold text-primary">72% cost optimization</span>
            </motion.p>
            ...
            >
              Built on Claude Sonnet 4.6 and Haiku 4.5, our platform delivers enterprise-grade AI
              for coding productivity, multilingual customer service, and intelligent game operations —
              all running on AWS infrastructure with smart model routing.
            </motion.p>
            ...
            >
              {[
                ...
                { value: "$200K", label: "Monthly MRR", sublabel: "Platform revenue" },
                { value: "72%", label: "Cost Saved", sublabel: "vs base pricing" },
                { value: "200+", label: "Engineers", sublabel: "Using AI daily" },
              ].map((stat, index) => (
                <motion.div
                  key={stat.label}
              ...
              >
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl lg:text-2xl font-bold text-foreground">{scenario.title}</h3>
                    <p className="text-sm text-muted-foreground mt-1">{scenario.subtitle}</p>
                  </div>
                  <div className="hidden md:block text-right">
                    <div className="text-2xl font-bold text-gradient">{scenario.dailyTokens}</div>
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-6 text-sm">
                  <div className="p-4 rounded-xl bg-secondary/50 border border-border/50">
                    ...
                    <div className="text-foreground mt-1 font-medium">{scenario.model}</div>
                  </div>
                  <div className="p-4 rounded-xl bg-secondary/50 border border-border/50">
                    <span className="text-xs uppercase tracking-wider text-muted-foreground">Tech Stack</span>
                    <div className="text-foreground mt-1 font-medium">{scenario.techStack}</div>
                  </div>
                </div>

                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {scenario.capabilities.map((cap) => (
                    <div key={cap.label} className="flex items-start gap-3 p-4 rounded-xl border border-border/50 bg-white hover:border-primary/20 transition-colors">
                      <CheckCircle2 className="w-4 h-4 text-primary mt-0.5 shrink-0" />
                      <div>
                        <div className="text-sm font-medium text-foreground">{cap.label}</div>
                        <div className="text-xs text-muted-foreground mt-0.5">{cap.detail}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          >
            <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-soft">
              <div className="grid grid-cols-4 gap-4 p-5 bg-gradient-to-r from-primary/5 to-accent/5 border-b border-border text-sm font-semibold">
                ...
                <div className="text-center text-primary">Claude Sonnet 4.6</div>
                <div className="text-center text-muted-foreground">Qwen-Max</div>
              ...
              {comparisonData.map((row, index) => (
                ...
                >
                  <div className={`text-sm ${row.highlight ? "font-bold text-foreground" : "text-foreground"}`}>
                    {row.scenario}
                  </div>
                  <div className={`text-center text-sm ${row.highlight ? "font-bold text-primary" : "text-primary font-medium"}`}>
                    {row.claude}
                  </div>
                  <div className="text-center text-sm text-muted-foreground">{row.qwen}</div>
                  <div className={`text-center text-sm font-medium ${row.highlight ? "text-primary font-bold" : "text-primary"}`}>
                    {row.gap}
                  </div>
                </motion.div>
              ))}
            ...
            >
              <p className="text-sm text-muted-foreground">
                ...
                Claude achieves a weighted overall score of <span className="font-bold text-primary">92.3</span> vs Qwen's 56.5 — a <span className="font-bold text-primary">+63%</span> improvement that directly translates to better player experiences.
              </p>
            </motion.div>
          </motion.div>
        </div>
      ...
      <section className="py-24 border-t border-border">
        <div className="container mx-auto px-6 lg:px-12">
          ...
          <div className="max-w-4xl mx-auto">
              ...
              {architecture.map((layer, index) => (
                ...
                >
                  <div className="flex items-center gap-5 p-6 rounded-xl bg-card border border-border hover:border-primary/20 hover:shadow-soft transition-all duration-300 group">
                    ...
                    <div className="flex-1">
                      <div className="font-semibold text-foreground text-lg">{layer.layer}</div>
                      <div className="text-sm text-muted-foreground mt-0.5">{layer.detail}</div>
                    </div>
            ...
            >
              {[
                { label: "Dual AZ Deployment", desc: "High availability across availability zones" },
                { label: "Smart Routing", desc: "95% requests to Haiku for cost efficiency" },
                { label: "45% Cache Hit Rate", desc: "Redis response caching reduces AI calls" },
              ].map((item) => (
                <div key={item.label} className="p-4 rounded-xl bg-secondary/50 border border-border/50 text-center">
          ...
          <div className="max-w-5xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-8">
              ...
              >
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between p-4 rounded-xl bg-secondary/50 border border-border/50">
                    <span className="text-muted-foreground">Base Price (no optimization)</span>
                    <span className="text-xl font-bold text-foreground">$723K/mo</span>
                  </div>

                  <div className="space-y-3 pl-4 border-l-2 border-primary/30">
                    <div className="flex items-center gap-3">
                      <TrendingDown className="w-4 h-4 text-primary shrink-0" />
                      <span className="text-sm text-muted-foreground">Prompt Cache (Sonnet 45% hit, Haiku 78% hit)</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <TrendingDown className="w-4 h-4 text-primary shrink-0" />
                      <span className="text-sm text-muted-foreground">ElastiCache response cache (45% hit rate)</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <TrendingDown className="w-4 h-4 text-primary shrink-0" />
                      <span className="text-sm text-muted-foreground">Smart model routing (95% requests → Haiku)</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <TrendingDown className="w-4 h-4 text-primary shrink-0" />
                      <span className="text-sm text-muted-foreground">Batch processing for non-realtime workloads</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-primary/10 to-accent/10 border border-primary/20">
                    ...
                    <span className="text-2xl font-bold text-gradient">~$200K/mo</span>
                  </div>

                  <div className="text-center text-sm text-muted-foreground">
                    Total Savings: <span className="text-primary font-semibold">$523K/month (72% reduction)</span>
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="space-y-4"
              >
                <h3 className="text-lg font-semibold text-foreground mb-2">Optimization Strategies</h3>
                {[
                  {
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs font-medium text-primary bg-primary/10 px-2 py-1 rounded-full">{strategy.saving}</span>
                    </div>
              ...
              {timeline.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-center">
                    <div className={`w-5 h-5 rounded-full shrink-0 ${index === 0 ? "bg-primary" : "bg-border"} border-2 border-primary shadow-sm`} />
                    {index < timeline.length - 1 && <div className="w-0.5 flex-1 bg-gradient-to-b from-primary/30 to-border" />}
                  </div>
                  ...
                  <div className="pb-8 flex-1">
                    <div className="bg-card border border-border rounded-xl p-6 hover:border-primary/20 hover:shadow-soft transition-all duration-300">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-lg font-bold text-foreground">{item.phase}</span>
                        <span className="text-sm font-semibold text-primary bg-primary/10 px-3 py-1 rounded-full">{item.mrr}</span>
                      </div>
